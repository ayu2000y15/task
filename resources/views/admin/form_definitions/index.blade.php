...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">カスタム項目定義 管理</h1>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-12"></th> {{-- ドラッグハンドル用 --}}
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700" id="sortable-definitions">
                    @forelse($fieldDefinitions as $definition)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50" data-id="{{ $definition->id }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $definition->order }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    ...
                                    <x-icon-button :href="route('admin.form-definitions.edit', $definition)" icon="fas fa-edit" title="編集" color="blue" />
                                    @endcan
                                    @can('delete', $definition)
                                    <form action="{{ route('admin.form-definitions.destroy', $definition) }}" method="POST" onsubmit="return confirm('本当に削除しますか？この項目定義を使用しているプロジェクトがある場合、表示に影響が出る可能性があります。');">
                                        @csrf
                                        @method('DELETE')
