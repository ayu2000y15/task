...
@section('title', 'フィードバックカテゴリ作成')

@section('content')
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
            <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">フィードバックカテゴリ作成</h1>
        ...
        <div class="max-w-xl mx-auto bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
            <div class="p-6 sm:p-8">
                <form action="{{ route('admin.feedback-categories.store') }}" method="POST">
                    @csrf
                    <div class="grid grid-cols-1 gap-y-6">
                        <div>
                            <x-input-label for="name" value="カテゴリ名" :required="true" />
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')"
                                required :hasError="$errors->has('name')" placeholder="例: 新規機能要望" />
