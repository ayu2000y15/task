    ...
    <div class="p-4 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
        ...
        <div class="flex justify-between items-start mb-2">
            <div class="flex items-center gap-x-3 min-w-0">
                <h3 class="font-semibold text-gray-800 dark:text-gray-200 truncate pr-2">{{ $request->title }}</h3>

                @if(is_null($request->completed_at) && !is_null($request->end_at) && $request->end_at->isPast())
                    <span
                        class="flex-shrink-0 inline-block px-2 py-0.5 text-xs font-medium text-red-800 bg-red-100 rounded-full dark:bg-red-900 dark:text-red-200">
                        期限切れ
                    </span>
                @endif
            </div>
            ...
            <div class="flex-shrink-0 flex items-center space-x-3">
                ...
                @can('delete', $request)
                    <form action="{{ route('requests.destroy', $request) }}" method="POST"
                        onsubmit="return confirm('この予定・依頼を本当に削除しますか？');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
        ...
        <div class="grid grid-cols-2 sm:flex sm:flex-wrap gap-x-4 gap-y-2 text-xs text-gray-500 dark:text-gray-400">
            <div class="flex items-center" title="作成者">
            ...
            @endif
            <div class="flex items-center" title="登録日">
                <i class="fas fa-calendar-alt w-4 text-center mr-1.5"></i>
                <span>{{ $request->created_at->format('Y/m/d') }}</span>
            </div>
            {{-- 依頼自体の日時表示 --}}
            @if($request->start_at)
                <div class="flex items-center col-span-2" title="日時">
                    <i class="far fa-clock w-4 text-center mr-1.5 text-blue-500"></i>
                    <span class="font-medium text-gray-600 dark:text-gray-300">
                        {{ $request->start_at->format('Y/m/d H:i') }}
                        @if($request->end_at && $request->end_at->format('Y-m-d') != $request->start_at->format('Y-m-d'))
                            〜 {{ $request->end_at->format('Y/m/d H:i') }}
                        @elseif($request->end_at)
                            〜 {{ $request->end_at->format('H:i') }}
                        @endif
                    </span>
                </div>
            @endif
