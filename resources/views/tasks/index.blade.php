...
@section('content')
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8"
            ...
            <div class="flex items-center flex-wrap gap-2">
                ...
                @php
                    ...
                    if ($isHidingCompleted) {
                        ...
                        $buttonText = '完了を表示'; $buttonIcon = 'fa-eye'; $buttonClass = $activeClass;
                    } else {
                        $hideCompletedParams['hide_completed'] = 1;
                        $buttonText = '完了を非表示'; $buttonIcon = 'fa-eye-slash'; $buttonClass = $inactiveClass;
                    }
                @endphp
        ...
        <div x-show="filtersOpen" x-collapse class="mb-6">
            <x-filter-panel :action="route('tasks.index')" :filters="$filters" :all-projects="$allProjects" :all-characters="$charactersForFilter" :all-assignees="$assigneesForFilter" :status-options="$statusOptions" :show-due-date-filter="true" />
        </div>
        ...
        <div class="mb-6" x-data="{ activeTab: 'tasks', listView: 'list' }">
            <div class="border-b border-gray-200 dark:border-gray-700">
                <nav class="-mb-px flex space-x-4 sm:space-x-8 overflow-x-auto pb-px" aria-label="Tabs">
                    <button class="tab-button py-4 px-1 inline-flex items-center gap-x-2 border-b-2 text-sm whitespace-nowrap focus:outline-none" :class="{ 'font-semibold border-blue-600 text-blue-600 dark:text-blue-500': activeTab === 'tasks', 'border-transparent text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-500': activeTab !== 'tasks' }" x-on:click="activeTab = 'tasks'">
                        <i class="fas fa-tasks mr-1"></i> 工程 ({{ $tasks->where('is_milestone', false)->where('is_folder', false)->count() }})
                    </button>
                    <button class="tab-button py-4 px-1 inline-flex items-center gap-x-2 border-b-2 text-sm whitespace-nowrap focus:outline-none" :class="{ 'font-semibold border-blue-600 text-blue-600 dark:text-blue-500': activeTab === 'milestones', 'border-transparent text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-500': activeTab !== 'milestones' }" x-on:click="activeTab = 'milestones'">
    ...
    <div x-data="{ isModalOpen: false, currentTaskId: null, assigneesForTask: [], selectedAssignees: [] }" x-show="isModalOpen" x-on:open-assignee-modal.window="isModalOpen = true; currentTaskId = $event.detail.taskId; assigneesForTask = $event.detail.assignees; selectedAssignees = []" x-on:keydown.escape.window="isModalOpen = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto z-50 flex items-center justify-center" style="display: none;">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl overflow-hidden sm:max-w-md sm:w-full" @click.away="isModalOpen = false">
            ...
            <div class="p-4 sm:p-6 max-h-60 overflow-y-auto">
                <template x-for="assignee in assigneesForTask" :key="assignee.id">
                    <div class="mb-2">
                        <label :for="'assignee-' + assignee.id + '-' + currentTaskId" class="inline-flex items-center w-full p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                            <input type="checkbox" :id="'assignee-' + assignee.id + '-' + currentTaskId" :value="assignee.id" x-model="selectedAssignees" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:bg-gray-900 dark:border-gray-600 dark:focus:ring-indigo-600 dark:focus:ring-offset-gray-800">
            ...
            <div class="px-4 py-3 sm:px-6 bg-gray-100 dark:bg-gray-700 text-right">
                <button type="button" class="inline-flex justify-center px-4 py-2 bg-gray-200 dark:bg-gray-600 border border-transparent rounded-md font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:text-sm" @click="isModalOpen = false">キャンセル</button>
                <button type="button" class="inline-flex justify-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:text-sm" @click="handleStartTimerWithSelection(currentTaskId, selectedAssignees); isModalOpen = false">開始</button>
            </div>
        </div>
    </div>
@endsection
