<div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden flex flex-col" x-data="{ open: false }">
    <div @click="open = !open" class="p-4 cursor-pointer transition-colors"
        style="background-color: {{ $project->color ?? '#6c757d' }}20;">
        <div class="flex items-center justify-between mb-2">
            <div class="flex items-center gap-2 flex-1 min-w-0 px-3 py-2 rounded"
        ...
        <div class="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-2 flex-wrap">
            ...
            @if($project->end_date->isPast() && !in_array($project->status, ['completed', 'cancelled']))
                <span
                    class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-red-500 text-white animate-pulse"
                    title="期限超過">
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200"
        ...
        x-transition:leave-end="opacity-0" class="border-t border-gray-200 dark:border-gray-700">
        <div class="p-5 space-y-4">
            ...
            <div class="flex flex-wrap gap-2 items-center text-xs">
                ...
                @endphp
                <span title="納品状況"
                    class="inline-flex items-center font-medium px-2 py-1 rounded-md {{ $deliveryColor }}"><i
                        class="fas fa-truck mr-1.5"></i> {{ $deliveryLabel }}</span>
                @if($project->payment_flag)
                    <span title="支払い状況"
                        class="inline-flex items-center font-medium px-2 py-1 rounded-md {{ $paymentColor }}"><i
