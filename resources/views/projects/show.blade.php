                ...
                <div class="mb-3">
                    ...
                    <ul class="list-group mt-2">
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            未着手
                            <span class="badge bg-secondary rounded-pill">{{ $project->tasks->where('status', 'not_started')->count() }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            進行中
                            <span class="badge bg-primary rounded-pill">{{ $project->tasks->where('status', 'in_progress')->count() }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            完了
                            <span class="badge bg-success rounded-pill">{{ $project->tasks->where('status', 'completed')->count() }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            保留中
                            <span class="badge bg-warning rounded-pill">{{ $project->tasks->where('status', 'on_hold')->count() }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            キャンセル
                            <span class="badge bg-danger rounded-pill">{{ $project->tasks->where('status', 'cancelled')->count() }}</span>
                        </li>
                    </ul>
                </div>
                <div class="mb-3">
                    ...
                    <ul class="list-group mt-2">
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            通常タスク
                            <span class="badge bg-primary rounded-pill">{{ $project->tasks->where('is_milestone', false)->where('is_folder', false)->count() }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            マイルストーン
                            <span class="badge bg-info rounded-pill">{{ $project->tasks->where('is_milestone', true)->count() }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            フォルダ
                            <span class="badge bg-secondary rounded-pill">{{ $project->tasks->where('is_folder', true)->count() }}</span>
                        </li>
                    </ul>
                </div>
            </div>
                            ...
                            @else
                                @foreach($project->tasks->sortBy('start_date') as $task)
                                    <tr>
                                        ...
                                        <td>{{ $task->assignee ?? '-' }}</td>
                                        <td>{{ $task->start_date->format('Y/m/d') }} 〜 {{ $task->end_date->format('Y/m/d') }}</td>
