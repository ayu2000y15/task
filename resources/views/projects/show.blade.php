...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8" id="project-show-main-container" data-project-id="{{ $project->id }}">
    ...
    <div class="mb-6 p-4 sm:p-6 rounded-lg shadow-lg text-white" style="background: linear-gradient(135deg, {{ $project->color }}DD, {{ $project->color }}FF); border-left: 4px solid {{ $project->color }};">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center" style="background: linear-gradient(135deg, {{ $project->color }}1A, {{ $project->color }}0A); border-left: 4px solid {{ $project->color }};">
                    <i class="fas fa-info-circle mr-2" style="color: {{ $project->color }};"></i>
                    <h5 class="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-0">案件情報</h5>
                </div>
                <div class="p-5 space-y-3">
                    <div class="flex justify-between items-start">
                        <span class="text-sm font-semibold text-gray-500 dark:text-gray-400 w-20 flex-shrink-0">作品名</span>
                        <span class="text-sm text-gray-700 dark:text-gray-300 text-right">{{ $project->series_title ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between items-start">
                        <span class="text-sm font-semibold text-gray-500 dark:text-gray-400 w-20 flex-shrink-0">依頼主</span>
                        <span class="text-sm text-gray-700 dark:text-gray-300 text-right">{{ $project->client_name ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between items-start">
                        <span class="text-sm font-semibold text-gray-500 dark:text-gray-400 w-20 flex-shrink-0">期間</span>
                        <span class="text-sm text-gray-700 dark:text-gray-300 text-right">
                            {{ $project->start_date->format('Y/m/d') }}<br>
                            <span class="text-xs text-gray-400 dark:text-gray-500">〜 {{ $project->end_date->format('Y/m/d') }}</span>
                        </span>
                    </div>
                    @if($project->description)
                        <div class="pt-2">
                            <span class="text-sm font-semibold text-gray-500 dark:text-gray-400 block mb-1">備考</span>
                            <p class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ $project->description }}</p>
                        </div>
                    @endif
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="text-right">
                            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mb-1">
                                <div class="h-2 rounded-full" style="width: {{ $progress }}%; background-color: {{ $project->color }};"></div>
                            </div>
                            <span class="text-xs font-semibold px-2 py-0.5 rounded-full" style="background-color: {{ $project->color }}; color:white;">{{ $progress }}%</span>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
                ...
                <div class="p-5">
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-4">
                        <div class="text-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-md">
                            <div class="text-xl font-bold text-gray-500 dark:text-gray-400">{{ $project->tasks()->where('is_milestone', false)->where('is_folder', false)->where('status', 'not_started')->count() }}</div>
                            <small class="text-gray-500 dark:text-gray-400">未着手</small>
                        </div>
                        <div class="text-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-md">
                            <div class="text-xl font-bold text-blue-500 dark:text-blue-400">{{ $project->tasks()->where('is_milestone', false)->where('is_folder', false)->where('status', 'in_progress')->count() }}</div>
                            <small class="text-gray-500 dark:text-gray-400">進行中</small>
                        </div>
                        <div class="text-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-md">
                            <div class="text-xl font-bold text-green-500 dark:text-green-400">{{ $project->tasks()->where('is_milestone', false)->where('is_folder', false)->where('status', 'completed')->count() }}</div>
                            <small class="text-gray-500 dark:text-gray-400">完了</small>
                        </div>
                        <div class="text-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-md">
                            <div class="text-xl font-bold text-yellow-500 dark:text-yellow-400">{{ $project->tasks()->where('is_milestone', false)->where('is_folder', false)->where('status', 'on_hold')->count() }}</div>
                            <small class="text-gray-500 dark:text-gray-400">保留中</small>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between items-center text-sm">
                            <span class="text-gray-600 dark:text-gray-300">通常工程</span>
                            <span class="px-2 py-0.5 text-xs font-semibold text-blue-800 bg-blue-100 dark:bg-blue-700 dark:text-blue-200 rounded-full">{{ $project->tasks->where('is_milestone', false)->where('is_folder', false)->count() }}</span>
                        </div>
                        <div class="flex justify-between items-center text-sm">
                            <span class="text-gray-600 dark:text-gray-300">重要納期</span>
                            <span class="px-2 py-0.5 text-xs font-semibold text-red-800 bg-red-100 dark:bg-red-700 dark:text-red-200 rounded-full">{{ $project->tasks->where('is_milestone', true)->count() }}</span>
                        </div>
                        <div class="flex justify-between items-center text-sm">
                            <span class="text-gray-600 dark:text-gray-300">フォルダ</span>
                            <span class="px-2 py-0.5 text-xs font-semibold text-gray-800 bg-gray-100 dark:bg-gray-600 dark:text-gray-200 rounded-full">{{ $project->tasks->where('is_folder', true)->count() }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div x-data="{
                ...
                <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center cursor-pointer" @click="expanded = !expanded">
                    <div class="flex items-center">
                        <i class="fas fa-users mr-2 text-gray-600 dark:text-gray-300"></i>
                        <h5 class="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-0">登場キャラクター</h5>
                    </div>
                ...
                <div x-show="expanded" x-collapse class="p-1 sm:p-3 md:p-5 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <form action="{{ route('projects.characters.store', $project) }}" method="POST">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-5 gap-4 items-end">
                                    <div class="sm:col-span-2">
                                        ...
                                        <input type="text" name="name" id="new_character_name"
                                            class="form-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-200"
                                            placeholder="例: 主人公" required value="{{ old('name') }}">
                    ...
                    @if($project->characters->isEmpty())
                        <div class="text-center py-10">
                            <i class="fas fa-user-plus text-4xl text-gray-400 dark:text-gray-500 mb-3"></i>
                            <h6 class="text-md font-semibold text-gray-700 dark:text-gray-300">キャラクターが登録されていません</h6>
                            <p class="text-sm text-gray-500 dark:text-gray-400">上のフォームから新しいキャラクターを追加してください。</p>
                        </div>
                    @else
                        <div class="space-y-6">
                            @foreach($project->characters as $character)
                                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden" >
                                    <div class="px-5 py-3 flex justify-between items-center border-b dark:border-gray-700" style="background: linear-gradient(135deg, {{ $project->color }}1A, {{ $project->color }}0A); border-left: 3px solid {{ $project->color }};">
                                        <h6 class="text-md font-semibold text-gray-800 dark:text-gray-100 truncate" title="{{ $character->name }}">
                                            <i class="fas fa-user mr-2" style="color: {{ $project->color }};"></i>
                                            {{ $character->name }}
                                        </h6>
                                        @can('update', $project)
                                        <div class="flex space-x-1 flex-shrink-0">
                                            ...
                                            <form action="{{ route('characters.destroy', $character) }}" method="POST" class="inline-block">
                                                <x-icon-button
                                                    ...
                                                    color="red"
                                                    size="sm"
                                                    type="submit" {{-- type="submit" を明示 --}}
                                                    method="DELETE" {{-- 実際にはformのmethodなので、コンポーネント内で @method を出力 --}}
                                                    :confirm="'このキャラクターを削除しますか？関連データも全て削除されます。'"
                                                />
                                            </form>
                                        </div>
                                        @endcan
                                    ...
                                    <div class="px-5 py-2 text-xs text-gray-600 dark:text-gray-400 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
                                    ...
                                    <div class="p-1 sm:p-2">
                                        <div class="border-b border-gray-200 dark:border-gray-700">
                                            <nav class="-mb-px flex space-x-2 sm:space-x-4 overflow-x-auto text-xs sm:text-sm" aria-label="Character Tabs for {{ $character->id }}">
                                                ...
                                                <button @click="activeCharacterTab[{{ $character->id }}] = 'measurements-{{ $character->id }}'"
                                                    ...
                                                    <i class="fas fa-ruler mr-1"></i> 採寸
                                                </button>
                                                @endcan
                                                @can('manageMaterials', $project)
                                                <button @click="activeCharacterTab[{{ $character->id }}] = 'materials-{{ $character->id }}'"
                                                        ...
                                                        class="whitespace-nowrap py-2 px-2 sm:px-3 border-b-2 font-medium focus:outline-none">
                                                    <i class="fas fa-box mr-1"></i> 材料
                                                </button>
                                                @endcan
                                                @can('viewAny', App\Models\Task::class)
                                                <button @click="activeCharacterTab[{{ $character->id }}] = 'tasks-{{ $character->id }}'"
                                                ...
                                                <button @click="activeCharacterTab[{{ $character->id }}] = 'costs-{{ $character->id }}'"
                                                        ...
                                                        class="whitespace-nowrap py-2 px-2 sm:px-3 border-b-2 font-medium focus:outline-none">
                                                    <i class="fas fa-yen-sign mr-1"></i> コスト
                                                </button>
                                                @endcan
                                            </nav>
                                        </div>
                                        <div class="py-3">
                                            @can('manageMeasurements', $project)
                                            <div x-show="activeCharacterTab[{{ $character->id }}] === 'measurements-{{ $character->id }}'" id="measurements-content-{{ $character->id }}">
                                                @include('projects.partials.character-measurements-tailwind', ['character' => $character, 'project' => $project])
                                            </div>
                                            @endcan
                                            @can('manageMaterials', $project)
                                            <div x-show="activeCharacterTab[{{ $character->id }}] === 'materials-{{ $character->id }}'" id="materials-content-{{ $character->id }}" style="display: none;">
                                                 @include('projects.partials.character-materials-tailwind', ['character' => $character, 'project' => $project])
                                            </div>
                                            @endcan
                                            @can('viewAny', App\Models\Task::class)
                                            <div x-show="activeCharacterTab[{{ $character->id }}] === 'tasks-{{ $character->id }}'" id="tasks-content-{{ $character->id }}" style="display: none;">
                                            ...
                                            @endcan
                                            @can('manageCosts', $project)
                                            <div x-show="activeCharacterTab[{{ $character->id }}] === 'costs-{{ $character->id }}'" id="costs-content-{{ $character->id }}" style="display: none;" class="character-costs-list-container" data-character-id="{{ $character->id }}">
            ...
            <div x-data="{ expanded: true }" class="bg-white dark:bg-gray-800 shadow-md rounded-lg">
                 <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center cursor-pointer" @click="expanded = !expanded">
                    <div class="flex items-center">
                        <i class="fas fa-tasks mr-2 text-gray-600 dark:text-gray-300"></i>
                        <h5 class="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-0">工程一覧 (案件全体)</h5>
                    </div>
                    <div class="text-sm text-gray-500 dark:text-gray-400">
                         <span class="mr-2">{{ $project->tasksWithoutCharacter->count() }}件</span>
                        <i class="fas" :class="expanded ? 'fa-chevron-up' : 'fa-chevron-down'"></i>
                    </div>
                </div>
                <div x-show="expanded" x-collapse class="border-t border-gray-200 dark:border-gray-700">
                    <div class="overflow-x-auto max-h-[60vh] overflow-y-auto">
                        @include('tasks.partials.task-table', ['tasksToList' => $project->tasksWithoutCharacter()->orderByRaw('ISNULL(start_date), start_date ASC, name ASC')->get(), 'tableId' => 'project-tasks-table', 'projectForTable' => $project, 'isProjectTaskView' => true])
                    </div>
                </div>
            </div>
        </div>
