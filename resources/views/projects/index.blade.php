    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @else
            ...
            <div x-data="{ open: false }">
                <h2 @click="open = !open" class="text-xl font-semibold text-gray-700 dark:text-gray-300 mb-4 border-b pb-2 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition">
                    <i class="fas fa-fw text-sm" :class="{'fa-chevron-right': !open, 'fa-chevron-down': open}"></i>
                    完了・キャンセルした案件 ({{ $archivedProjects->count() }})
                </h2>

                <div x-show="open" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform -translate-y-4" x-transition:enter-end="opacity-100 transform translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 transform translate-y-0" x-transition:leave-end="opacity-0 transform -translate-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
