...
@section('content')
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8"
        x-data="{ filtersOpen: {{ array_filter($filters) ? 'true' : 'false' }}, detailsVisible: true }">
        ...
        @if ($projects->isEmpty())
            <div class="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 rounded-md shadow-sm dark:bg-blue-700 dark:text-blue-100 dark:border-blue-300"
                role="alert">
                表示する案件がありません。フィルター条件を変更するか、新規案件を作成してください。
            </div>
        @else
            <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
                <div class="gantt-container overflow-x-auto relative">
                    <div class="gantt-scroll-container relative">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border-collapse" id="ganttTable"
                            :class="{ 'details-hidden': !detailsVisible }">
                            <thead class="gantt-header text-xs text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                <tr>
                                    <th rowspan="2" class="gantt-sticky-col px-3 py-3 align-top font-medium"
                                        style="min-width: 300px;">案件</th>
                                    <th rowspan="2" class="detail-column px-3 py-3 align-top font-medium"
                                        style="min-width: 70px;">開始日</th>
                                    <th rowspan="2" class="detail-column px-3 py-3 align-top font-medium"
                                        style="min-width: 70px;">終了日</th>
                                    <th rowspan="2" class="detail-column px-3 py-3 align-top font-medium"
                                        style="min-width: 80px;">ステータス</th>
                                    @php
                                        $months = [];
                                    ...
                                    @foreach($months as $month)
                                        <th colspan="{{ $month['count'] }}"
                                            class="text-center py-1.5 font-medium border-b border-gray-200 dark:border-gray-700">
                                            {{ $month['name'] }}</th>
                                    @endforeach
                                </tr>
                                <tr>
                                    @foreach($dates as $dateInfo)
                                        @php
                                            ...
                                            if ($dateInfo['is_saturday'])
                                                $classes[] = 'saturday'; // Handled by app.css
                                            elseif ($dateInfo['is_sunday'] || $isHoliday)
                                                $classes[] = 'sunday'; // Handled by app.css
                                            if ($dateInfo['date']->isSameDay($today))
                                                $classes[] = 'today'; // Handled by app.css
                                        @endphp
                                        <th class="gantt-cell {{ implode(' ', $classes) }} font-normal border border-gray-200 dark:border-gray-700"
                                            style="min-width: 35px;" title="{{ $isHoliday ? $holidays[$dateStr]->name : '' }}"
                                            data-date="{{ $dateStr }}">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                                @foreach($projects as $project)
                                    <tr
                                        class="project-header project-{{ $project->id }}-tasks task-level-0 bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700">
                                            ...
                                            <div class="flex justify-between items-start">
                                                <div class="flex items-start gantt-task-name-container">
                                                    ...
                                                    <a href="{{ route('projects.show', $project) }}"
                                                        class="font-medium text-gray-800 dark:text-gray-100 hover:text-blue-600 dark:hover:text-blue-400 gantt-task-name-link">{{ $project->title }}</a>
                                                ...
                                                <div class="task-actions flex space-x-1 flex-shrink-0">
                                                    ...
                                                    <a href="{{ route('projects.edit', $project) }}"
                                                        class="p-1 text-yellow-500 hover:text-yellow-700 dark:text-yellow-400 dark:hover:text-yellow-300"
                                                        title="編集">
                                        ...
                                        @for($i = 0; $i < count($dates); $i++)
                                            ...
                                            @endphp
                                            <td class="gantt-cell {{ implode(' ', $cellClasses) }} p-0 relative border-x border-gray-200 dark:border-gray-700"
                                                data-date="{{ $dateStr }}" style="{{ $style }}">
                                                @if($hasBar)
        ...
        <x-modal name="ganttFileUploadModal" maxWidth="2xl">
            <div class="p-6" x-data="{ modalProjectId: null, modalTaskId: null, modalTaskName: '' }" @open-modal.window="if ($event.detail.name === 'ganttFileUploadModal') {
                ...
                <div class="flex justify-between items-center pb-3 border-b dark:border-gray-700">
                    ...
                    <button @click="$dispatch('close-modal', { name: 'ganttFileUploadModal' })"
                ...
                <div class="mt-4">
                    <form action="#" method="post" class="dropzone dropzone-custom-style mb-3"
                        id="gantt-file-upload-dropzone-alpine">
                        @csrf
                        <div class="dz-message text-center" data-dz-message>
                            ...
                            <button type="button" class="dz-button-bootstrap"><i
                                    class="fas fa-folder-open mr-1"></i>ファイルを選択</button>
                        </div>
                    </form>
                ...
                <div class="mt-6 flex justify-end space-x-2">
                    <x-secondary-button
                        @click="$dispatch('close-modal', { name: 'ganttFileUploadModal' })">閉じる</x-secondary-button>
                    <x-primary-button @click="Alpine.store('ganttDropzone').processQueue()"
                        id="ganttProcessUploadQueueBtnAlpine"><i
                            class="fas fa-upload mr-1"></i>選択ファイルをアップロード</x-primary-button>
                </div>
            </div>
        </x-modal>
    </div>
