...
@section('title', '購読者追加 - ' . $emailList->name)
...
@section('breadcrumbs')
    ...
    <span class="text-gray-500 dark:text-gray-400 mx-2">/</span>
    <span class="text-gray-700 dark:text-gray-200">購読者追加</span>
@endsection
        ...
        <div class="max-w-3xl mx-auto mb-6"> {{-- フォームの幅を少し広げる max-w-3xl --}}
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">
                    購読者追加 <span class="text-base font-normal text-gray-600 dark:text-gray-400">(リスト:
                        {{ $emailList->name }})</span>
                </h1>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <x-text-input type="text" id="company_name" name="company_name" class="mt-1 block w-full"
                                :value="old('company_name')" :hasError="$errors->has('company_name')" />
                        ...
                        <div>
                            ...
                            <x-text-input type="text" id="postal_code" name="postal_code" class="mt-1 block w-full"
                                :value="old('postal_code')" :hasError="$errors->has('postal_code')"
                                placeholder="123-4567" />
                            <x-input-error :messages="$errors->get('postal_code')" class="mt-2" />
                        </div>
                        <div class="md:col-span-2"> {{-- 住所は幅広く取る --}}
                            <x-input-label for="address" value="住所" />
                            <x-textarea-input id="address" name="address" class="mt-1 block w-full" rows="3"
                                :hasError="$errors->has('address')">{{ old('address') }}</x-textarea-input>
                        ...
                        <div>
                            ...
                            <x-text-input type="tel" id="phone_number" name="phone_number" class="mt-1 block w-full"
                                :value="old('phone_number')" :hasError="$errors->has('phone_number')"
                        ...
                        <div>
                            ...
                            <x-text-input type="tel" id="fax_number" name="fax_number" class="mt-1 block w-full"
                                :value="old('fax_number')" :hasError="$errors->has('fax_number')" />
                            <x-input-error :messages="$errors->get('fax_number')" class="mt-2" />
                        </div>
                        <div class="md:col-span-2">
                            <x-input-label for="url" value="URL" />
                            <x-text-input type="url" id="url" name="url" class="mt-1 block w-full" :value="old('url')"
                                :hasError="$errors->has('url')" placeholder="https://example.com" />
                        ...
                        <div>
                            ...
                            <x-text-input type="text" id="representative_name" name="representative_name"
                                class="mt-1 block w-full" :value="old('representative_name')"
                        ...
                        <div>
                            ...
                            <x-text-input type="date" id="establishment_date" name="establishment_date"
                                class="mt-1 block w-full" :value="old('establishment_date')"
                        ...
                        <div>
                            ...
                            <x-select-input label="ステータス" id="status" name="status" class="mt-1 block w-full"
                                :options="$statusOptions" :selected="old('status', 'subscribed')"
