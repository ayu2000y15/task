@extends('layouts.app')
@section('title', '交通費の登録')

@section('content')
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
            <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">交通費の登録</h1>
        ...
        <div class="max-w-xl mx-auto bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
            <form action="{{ route('transportation-expenses.store') }}" method="POST">
                @csrf
                <div class="space-y-6">
                    <div>
                        <x-input-label for="date" value="利用日" :required="true" />
                        <x-text-input id="date" name="date" type="date" class="mt-1 block w-full" :value="old('date', today()->format('Y-m-d'))" required />
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="departure" name="departure" type="text" class="mt-1 block w-full"
                                :value="old('departure')" placeholder="例: 自宅" />
                        ...
                        <div>
                            ...
                            <x-text-input id="destination" name="destination" type="text" class="mt-1 block w-full"
                                :value="old('destination')" required placeholder="例: 〇〇スタジオ" />
                    ...
                    <div class="flex justify-end space-x-3 pt-4 border-t dark:border-gray-700">
                        ...
                        <x-primary-button type="submit">登録する</x-primary-button>
                    </div>
                </div>
            </form>
        </div>
